...
# Enable persistent storage path configuration (useful for environments like Render/Railway)
...
def index():
    ...
    conn.close()
    return render_template("index.html", active_count=active_count, orders=orders, searched_roll_no=roll_no, shop_status=settings.get('shop_status', 'open'), shop_announcement=settings.get('shop_announcement', ''), settings=settings)
